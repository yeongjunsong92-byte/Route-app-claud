...
type RecentRegion = RegionSelection;
...
export default function Home() {
  ...
  const [recentRegions, setRecentRegions] = useState<RecentRegion[]>(() => {
    try {
      ...
      return Array.isArray(stored) ? stored.filter((region) => region && typeof region.label === "string" && typeof region.lat === "number" && typeof region.lng === "number").slice(0, 5) : [];
    } catch { return []; }
  });
  ...
  const publicCourses = useMemo<Course[]>(() => (publicCoursesQuery.data || []).map((course: any) => ({ id: String(course.id), title: course.title, region: course.region || "여행", author: course.authorName || "Route 여행자", image: course.coverImage || mockPlaces[0].image, likes: 0, days: 1, items: [], startDate: course.startDate, endDate: course.endDate, status: course.status || "planned" })), [publicCoursesQuery.data]);
  const followingPublicCourses = useMemo<Course[]>(() => (followingPublicCoursesQuery.data || []).map((course: any) => ({ id: String(course.id), title: course.title, region: course.region || "여행", author: course.authorName || "Route 여행자", image: course.coverImage || mockPlaces[0].image, likes: 0, days: 1, items: [], startDate: course.startDate, endDate: course.endDate, status: course.status || "planned" })), [followingPublicCoursesQuery.data]);
  ...
  const saveCourse = async () => {
    try {
      ...
      if (isCoursePublic) {
        setSelectedCourse({ id: String(courseId), title: courseTitle, region: "서울", author: user?.name || "나의 Route", image: coursePlaces[0]?.image || mockPlaces[0].image, likes: 0, days: 1, items: [], startDate: courseStartDate || null, endDate: courseEndDate || null, status: courseStatus, isPublic: true });
        setScreen("course-detail");
  ...
  const rememberRecentRegion = (region: RecentRegion) => {
    setRecentRegions((current) => [region, ...current.filter((item) => item.label !== region.label || Math.abs(item.lat - region.lat) > 0.00001 || Math.abs(item.lng - region.lng) > 0.00001)].slice(0, 5));
  };
