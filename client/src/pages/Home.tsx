...
export default function Home() {
  ...
  const renderNaverInstallHelpSheet = () => {
    ...
    const installUrl = isIOS
      ? "https://apps.apple.com/kr/app/%EB%84%A4%EC%9D%B4%EB%B2%84%EC%A7%80%EB%8F%84-%EC%9E%A5%EC%86%8C%EC%9D%98-%EB%B0%9C%EA%B2%AC%EA%B3%BC-%EC%98%88%EC%95%BD-%EB%82%B4%EB%B9%84%EA%B2%8C%EC%9D%B4%EC%85%98/id311867728"
      : "https://play.google.com/store/apps/details?id=com.nhn.android.nmap";
    const closeHelp = () => setIsNaverInstallHelpOpen(false);
    return <div className="route-overlay route-naver-install-overlay" onClick={closeHelp}><section className="route-naver-install-sheet" role="dialog" aria-modal="true" aria-label="네이버 내비 설치 안내" onClick={(event) => event.stopPropagation()}><div className="route-sheet-handle" /><div className="route-naver-install-icon"><span>N</span><Navigation size={20} /></div><span>NAVER NAVIGATION</span><h3>네이버 내비 앱이 없으신가요?</h3><p>앱을 설치하면 {navigationPlace.name}까지 실제 길안내를 바로 시작할 수 있어요. 설치하지 않아도 네이버지도 웹에서 목적지를 확인할 수 있습니다.</p><a className="route-naver-install-primary" href={installUrl} target="_blank" rel="noreferrer"><Download size={17} /> 네이버지도 앱 설치하기</a><a className="route-naver-install-web" href={naverMapSearchUrl(navigationPlace)} target="_blank" rel="noreferrer" onClick={closeHelp}><ExternalLink size={16} /> 네이버지도 웹에서 길찾기</a><button className="route-naver-install-cancel" onClick={closeHelp}>앱에서 다시 확인</button></section></div>;
  };
