...
function getPlacePhotos(place: Place) {
  const verifiedPhotos = place.photos?.filter((photo) => typeof photo === "string" && photo.length > 0) || [];
  if (verifiedPhotos.length) return verifiedPhotos;
...
function RouteMapEntryLoader({ isExiting = false }: { isExiting?: boolean }) {
  return <div className={`route-map-entry-loader${isExiting ? " is-exiting" : ""}`} role="status" aria-live="polite" aria-label="지도를 준비하고 있습니다"><div className="route-map-entry-loader-content"><div className="route-map-entry-loader-logo"><span className="route-map-entry-loader-orbit" aria-hidden="true" /><RouteBrandMark /></div><span className="route-map-entry-loader-pulse"><MapPin size={15} fill="currentColor" /></span><strong>나만의 여행 지도를 준비하고 있어요</strong><small>잠시만 기다려 주세요.</small></div></div>;
}
...
function PlaceVisual({ place, detail = false }: { place: Place; detail?: boolean }) {
  if (place.image) return <img src={place.image} alt="" />;
  return <span className={`route-place-image-empty route-place-mini-map${detail ? " is-detail" : ""}`} aria-label={`${place.name} 사진 없음, 위치 미리보기`}><span className="route-place-mini-map-grid" aria-hidden="true" /><span className="route-place-mini-map-route route-a" aria-hidden="true" /><span className="route-place-mini-map-route route-b" aria-hidden="true" /><MapPin size={detail ? 24 : 18} fill="currentColor" /><small>위치 미리보기</small><em>{place.lat.toFixed(3)}, {place.lng.toFixed(3)}</em></span>;
}

function PlaceRow({ place, onClick, onSave, distanceLabel = "350m", saveLabel = "저장" }: { place: Place; onClick: () => void; onSave: () => void; distanceLabel?: string; saveLabel?: string }) {
  ...
  return <div className="route-place-row" role="group" aria-label={`${place.name} 장소 작업`}><button className="route-place-main" onClick={onClick}><PlaceVisual place={place} /><span className="route-place-copy"><strong>{place.name}</strong><small>{hasGoogleRating ? `★ ${place.rating} · 리뷰 ${place.reviewCount} · ` : ""}{place.category}</small>{openingLabel && <em className={place.openNow === false ? "is-closed" : place.openNow === true ? "is-open" : ""}>{openingLabel}</em>}<i>{place.address || "주소 정보 없음"}</i></span><span className="route-place-distance">{distanceLabel}</span></button><button className="route-place-navigate" aria-label={`${place.name} 길찾기`} onClick={dispatchNavigation}><Navigation size={15} /></button><button className="route-place-save" aria-label={`${place.name} ${saveLabel}`} onClick={onSave}><Bookmark size={16} /></button></div>;
}
...
export default function Home() {
  ...
  const refreshSavedPlaceSourceMutation = trpc.places.refreshSource.useMutation();
  ...
  const [isMapEntryTransitionExiting, setIsMapEntryTransitionExiting] = useState(false);
  ...
  const [courseListTab, setCourseListTab] = useState<CourseListTab>("mine");
  const [publicCourseDetailBackScreen, setPublicCourseDetailBackScreen] = useState<Screen>("friends");
  ...
  const savedMapPlaces = useMemo<Place[]>(() => (savedPlacesQuery.data || []).flatMap((place: any) => {
    ...
    return [{
      ...
      description: "실제 Google Maps 저장 결과입니다.",
      rating: 0,
      reviewCount: 0,
      lat: place.lat,
      lng: place.lng,
  ...
  const savedPlaceRecordByPlaceId = useMemo(() => new Map((savedPlacesQuery.data || []).map((place: any) => [place.placeId, place])), [savedPlacesQuery.data]);
  ...
  const toCanonicalGooglePlace = (result: google.maps.places.PlaceResult): Place | null => {
    const placeId = result.place_id?.trim();
    const name = result.name?.trim();
    const lat = result.geometry?.location?.lat();
    const lng = result.geometry?.location?.lng();
    if (!placeId || !name || lat === undefined || lng === undefined) return null;
    const photos = result.photos?.slice(0, 5).map((photo) => photo.getUrl({ maxWidth: 1200, maxHeight: 900 })) || [];
    return {
      id: placeId,
      name,
      ...
      image: photos[0] || "",
      description: "실제 Google Maps 검색 결과입니다.",
      ...
      photos,
      openNow: getGooglePlaceOpenNow(result.opening_hours),
  ...
  const hydrateGooglePlaceDetails = (place: Place) => {
    ...
    service.getDetails({ placeId: place.id, fields: ["place_id", "name", "formatted_address", "formatted_phone_number", "geometry", "opening_hours", "photos", "rating", "user_ratings_total", "types", "website"] }, (details, status) => {
      if (status !== google.maps.places.PlacesServiceStatus.OK || !details) return;
      const canonical = toCanonicalGooglePlace(details);
      if (!canonical || canonical.id !== place.id) return;
      const enriched: Place = {
        ...place,
        ...canonical,
        image: canonical.image || place.image,
        photos: canonical.photos?.length ? canonical.photos : place.photos,
      };
      ...
      const savedRecord = savedPlaceRecordByPlaceId.get(place.id);
      if (savedRecord) {
        void refreshSavedPlaceSourceMutation.mutateAsync({
          savedPlaceId: savedRecord.id,
          name: enriched.name,
        ...
        }).then(() => trpcUtils.places.saved.invalidate()).catch(() => undefined);
      }
    });
  };
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      ...
      setIsMapEntryTransitionExiting(false);
      return;
    }
    ...
    setIsMapEntryTransitionExiting(false);
    const exitTimeout = window.setTimeout(() => setIsMapEntryTransitionExiting(true), 620);
    const removeTimeout = window.setTimeout(() => setIsMapEntryTransitionVisible(false), 960);
    return () => {
  ...
  const openPublicCourse = (course: Course, backScreen: Screen = "friends") => {
    setSelectedCourse(course);
    setPublicCourseDetailBackScreen(backScreen);
    if (backScreen === "friends") setSelectedTab("friends");
    if (backScreen === "my-courses") setSelectedTab("courses");
    setScreen("public-course-detail");
  };
  const openCourseLibrary = (tab: CourseListTab) => {
    setCourseListTab(tab);
    setSelectedTab("courses");
  ...
  const installRouteApp = async () => {
    if (!deferredInstallPrompt) {
      toast.message("브라우저 메뉴에서 ‘홈 화면에 추가’ 또는 ‘앱 설치’를 선택해 주세요.");
      return;
    }
    try {
      await deferredInstallPrompt.prompt();
      const choice = await deferredInstallPrompt.userChoice;
      if (choice?.outcome === "accepted") toast.success("Route를 홈 화면에 설치했습니다.");
      setDeferredInstallPrompt(null);
    } catch {
      toast.error("앱 설치 창을 열지 못했습니다. 브라우저 메뉴에서 홈 화면에 추가해 주세요.");
    }
  };
  ...
  const shareCourse = async () => {
    if (!isSelectedCoursePublic) {
      toast.message("전체 공개 코스로 전환한 뒤 다른 사람에게 공유할 수 있습니다.");
      return;
    }
    try {
  ...
  const shareSelectedPlace = async () => {
    if (!selectedPlace) return;
    const link = naverMapSearchUrl(selectedPlace);
    const text = `${selectedPlace.name} · ${selectedPlace.address}`;
    try {
      if (navigator.share) await navigator.share({ title: `${selectedPlace.name} | Route`, text, url: link });
      else if (navigator.clipboard?.writeText) {
        await navigator.clipboard.writeText(`${text}\n${link}`);
        toast.success("장소 링크를 복사했습니다.");
      } else window.prompt("장소 정보를 복사하세요.", `${text}\n${link}`);
    } catch { /* 사용자가 기기 공유 시트를 닫은 경우 별도 알림을 표시하지 않습니다. */ }
  };
  ...
  const searchPlaces = (keywordOverride = query) => {
    ...
    service.textSearch({ query: keyword, location: map.getCenter() || { lat: 37.5446, lng: 127.0557 }, radius: 7000 }, (results, status) => {
      ...
      const normalized = results.slice(0, 12).flatMap((result): Place[] => {
        const place = toCanonicalGooglePlace(result);
        return place ? [place] : [];
      });
      setHasLiveSearch(true);
  ...
  const choosePrediction = (prediction: google.maps.places.AutocompletePrediction) => {
    ...
    const map = mainMapRef.current;
    if (!map || !window.google?.maps?.places) {
      toast.error("지도가 준비된 후 다시 선택해주세요.");
      return;
    }
    setPlacesLoading(true);
    new google.maps.places.PlacesService(map).getDetails({ placeId: prediction.place_id, fields: ["place_id", "name", "formatted_address", "formatted_phone_number", "geometry", "opening_hours", "photos", "rating", "user_ratings_total", "types", "website"] }, (detail, status) => {
      setPlacesLoading(false);
      const place = status === google.maps.places.PlacesServiceStatus.OK && detail ? toCanonicalGooglePlace(detail) : null;
      if (!place || place.id !== prediction.place_id) {
        toast.error("선택한 장소의 정확한 정보를 불러오지 못했습니다. 다른 검색어로 다시 시도해 주세요.");
        return;
      }
      setHasLiveSearch(true);
      setLivePlaces([place]);
      ...
      map.setZoom(16);
      setRecentSearches((current) => {
        const next = [prediction.description, ...current.filter((item) => item !== prediction.description)].slice(0, 6);
        persistRecentSearches(next);
        return next;
      });
    });
  ...
  function loadNearbyPlaces(origin: { lat: number; lng: number }, category: string, isCurrentLocationSearch: boolean, keepFullscreen = false) {
    ...
    const finishNearbySearch = () => {
      ...
      const normalized = uniqueResults.flatMap((result): Place[] => {
        const place = toCanonicalGooglePlace(result);
        return place ? [place] : [];
      ...
      normalized.forEach((place) => {
        ...
        service.getDetails({ placeId: place.id, fields: ["place_id", "name", "formatted_address", "formatted_phone_number", "geometry", "opening_hours", "photos", "rating", "user_ratings_total", "types", "website"] }, (detail, detailStatus) => {
          if (detailStatus !== google.maps.places.PlacesServiceStatus.OK || !detail) return;
          const canonical = toCanonicalGooglePlace(detail);
          if (!canonical || canonical.id !== place.id) return;
          setLivePlaces((current) => current.map((item) => item.id === place.id ? { ...item, ...canonical, image: canonical.image || item.image, photos: canonical.photos?.length ? canonical.photos : item.photos } : item));
        });
      });
    };
  ...
  const renderPlaceDetail = () => {
    ...
    return <div className="route-screen route-detail-screen"><section className="route-detail-location-section" aria-label="장소 위치 지도"><div className="route-detail-map">{renderMap(true)}<button className="route-floating-back" aria-label="지도 화면으로 돌아가기" onClick={restoreMapFromPlaceDetail}><ArrowLeft size={18} /></button><button className="route-floating-share" aria-label="장소 공유" onClick={() => void shareSelectedPlace()}><Share2 size={16} /></button></div><div className="route-detail-location-summary"><span><MapPin size={15} /> 지도 위치</span><strong>{selectedPlace.address || "주소 정보 없음"}</strong></div></section><div className="route-place-detail-card"><section className="route-detail-photo-section" aria-label="장소 사진"><div className="route-detail-section-heading"><h3>{photos.length ? "사진" : "위치 미리보기"}</h3><span>{photos.length ? `${getPlacePhotos(selectedPlace).length}장` : "사진 없음"}</span></div><div className="route-detail-images">{photos.length ? photos.map((photo, index) => <button key={`${photo}-${index}`} aria-label={`${selectedPlace.name} 사진 ${index + 1} 확대`} onClick={() => { setGalleryIndex(index); setIsGalleryOpen(true); }}><img src={photo} alt={`${selectedPlace.name} 사진 ${index + 1}`} />{index === 2 && getPlacePhotos(selectedPlace).length > 3 && <span>+{getPlacePhotos(selectedPlace).length - 3}</span>}</button>) : <PlaceVisual place={selectedPlace} detail />}</div></section><div className="route-detail-body"><div className="route-detail-title-row"><div><h2>{selectedPlace.name}</h2><p>{typeof selectedPlace.rating === "number" && selectedPlace.rating > 0 && typeof selectedPlace.reviewCount === "number" && selectedPlace.reviewCount > 0 ? `★ ${selectedPlace.rating} (${selectedPlace.reviewCount}) · ` : ""}{selectedPlace.category}</p></div><button aria-label={`${selectedPlace.name} 저장`} onClick={() => openSaveSheet(selectedPlace)}><Bookmark size={18} /></button></div><section className="route-detail-intro"><h3>장소 소개</h3><p className="route-detail-description">{selectedPlace.description}</p></section>{(openingLabel || selectedPlace.phone) && <section className="route-detail-basic-info" aria-label="기본 정보"><h3>기본 정보</h3>{openingLabel && <p><span><Clock3 size={14} /> 영업시간</span><strong>{openingLabel}</strong></p>}{selectedPlace.phone && <p><span><Users size={14} /> 전화번호</span><strong>{selectedPlace.phone}</strong></p>}</section>}<div className="route-detail-external-links"><a className="route-naver-link" href={naverReservationUrl(selectedPlace)} target="_blank" rel="noreferrer"><ExternalLink size={15} /><span><strong>{naverReservationLabel(selectedPlace)}</strong><small>{selectedPlace.website ? "네이버 예약 또는 장소 페이지로 이동합니다." : "네이버 검색에서 예약·문의 가능 여부를 확인합니다."}</small></span><ChevronRight size={15} /></a><a className="route-naver-sub-link" href={naverMapSearchUrl(selectedPlace)} target="_blank" rel="noreferrer">네이버 지도에서 장소 보기 <ChevronRight size={13} /></a></div></div><div className="route-detail-actions"><button className="secondary" onClick={() => openSaveSheet(selectedPlace)}>저장</button><button onClick={() => openCoursePicker(selectedPlace)}>코스에 추가</button></div></div>{saveSheetOpen && renderSaveSheet()}</div>;
  };

  const renderPlaceDetailWithNavigation = () => <>{renderPlaceDetail()}{selectedPlace && <button className="route-place-navigation-fab" aria-label={`${selectedPlace.name} 길찾기`} onClick={() => openPlaceNavigation(selectedPlace)}><Navigation size={16} />길찾기</button>}</>;
  ...
  const renderRouteDirections = () => {
    ...
    const originStop: RouteStop = { name: navigationOrigin.label, lat: navigationOrigin.lat, lng: navigationOrigin.lng };
    const destinationStop: RouteStop = { name: navigationPlace.name, lat: navigationPlace.lat, lng: navigationPlace.lng };
    return <div className="route-screen route-place-navigation"><ScreenHeader title="길찾기" onBack={() => setScreen(navigationBackScreen || (selectedPlace ? "place-detail" : "map"))} /><section className="route-navigation-destination"><div className="route-navigation-origin"><LocateFixed size={15} /><span><small>출발</small><strong>{navigationOrigin.label}</strong><em>{navigationOrigin.address}</em></span><button aria-label="출발지 변경" onClick={() => setIsNavigationOriginEditorOpen((open) => !open)}><Pencil size={14} /> 변경</button></div><div className="route-navigation-line" /><div className="route-navigation-arrival"><MapPin size={16} /><span><small>도착</small><strong>{navigationPlace.name}</strong><em>{navigationPlace.address}</em></span></div></section>{isNavigationOriginEditorOpen && <section className="route-navigation-origin-editor" aria-label="출발지 지정"><div className="route-navigation-origin-editor-heading"><div><span>STARTING POINT</span><h3>출발지를 직접 지정하세요</h3></div><button aria-label="출발지 지정 닫기" onClick={() => setIsNavigationOriginEditorOpen(false)}><X size={15} /></button></div><div className="route-navigation-origin-input"><MapPin size={15} /><input aria-label="출발지 입력" value={navigationOriginQuery} onChange={(event) => setNavigationOriginQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") resolveNavigationOrigin(); }} placeholder="집, 학교, 주소를 입력하세요" /><button onClick={resolveNavigationOrigin}>적용</button></div><button className="route-navigation-current-origin" onClick={() => { setNavigationOriginOverride(null); setNavigationOriginQuery(""); if (!userLocation) moveToCurrentLocation(); }}><LocateFixed size={14} /> 현재 위치 사용</button></section>}{renderTravelModeSelector()}<DistanceOverviewMap key={`route-directions-${navigationPlace.id}-${navigationOrigin.lat}-${navigationOrigin.lng}`} origin={originStop} destination={destinationStop} /><section className="route-navigation-summary"><Icon size={20} /><div><small>{travelModeMeta[travelMode].summary}</small><strong>{formatMinutes(navigationMinutes)} · 직선 거리 {formatDistance(navigationDistanceMeters)}</strong><span>예상 시간은 직선 거리와 수단별 평균 속도를 기준으로 계산됩니다.</span></div></section><section className="route-external-navigation" aria-label="외부 지도 열기"><div><span>OPEN MAP</span><h3>원하는 지도 앱에서 확인하세요</h3></div><a href={googleNavigationUrl(navigationPlace, travelMode, navigationOrigin)} target="_blank" rel="noreferrer"><span className="google">G</span><div><strong>Google Maps</strong><small>{travelModeMeta[travelMode].label} 길찾기 열기</small></div><ExternalLink size={16} /></a><a href={kakaoNavigationUrl(navigationPlace)} target="_blank" rel="noreferrer"><span className="kakao">K</span><div><strong>카카오맵</strong><small>목적지 길찾기 열기</small></div><ExternalLink size={16} /></a></section></div>;
  };

  const renderRouteDirectionsWithShare = () => <>{renderRouteDirections()}{navigationPlace && <button className="route-directions-share-trigger" aria-label="길찾기 공유" onClick={() => setIsNavigationShareOpen(true)}><Share2 size={17} /></button>}{isNavigationShareOpen && navigationPlace && <div className="route-overlay route-course-share-overlay" onClick={() => setIsNavigationShareOpen(false)}><section className="route-course-share-sheet" role="region" aria-label="길찾기 공유" onClick={(event) => event.stopPropagation()}><div className="route-sheet-handle" /><div className="route-course-share-heading"><div><span>SHARE DIRECTIONS</span><h3>{navigationPlace.name} 길찾기</h3><p>{navigationShareText}</p></div><button aria-label="길찾기 공유 닫기" onClick={() => setIsNavigationShareOpen(false)}><X size={17} /></button></div><button aria-label="길찾기 링크 복사" onClick={() => void copyNavigationShareLink()}><span className="link"><Link2 size={19} /></span><div><strong>길찾기 링크 복사</strong><small>출발지·도착지·이동 수단을 함께 전달합니다.</small></div><Copy size={16} /></button><button aria-label="카카오톡으로 길찾기 공유" onClick={() => void shareNavigation()}><span className="native"><Share2 size={19} /></span><div><strong>카카오톡으로 공유</strong><small>기기 공유 시트에서 카카오톡을 선택할 수 있습니다.</small></div><ChevronRight size={16} /></button></section></div>}</>;
  ...
  const renderMyPlaces = () => {
    ...
    return <div className="route-screen route-list-screen"><ScreenHeader title="내 장소" onBack={() => setScreen("map")} /><div className="route-list-tabs" role="tablist" aria-label="내 장소 카테고리">{categories.map((category) => <button key={category} role="tab" aria-selected={myPlacesCategory === category} className={myPlacesCategory === category ? "active" : ""} onClick={() => setMyPlacesCategory(category)}>{category}</button>)}</div><div className="route-list-content">{savedPlacesQuery.isLoading ? <div className="route-empty"><Bookmark size={22} /><strong>저장 장소를 불러오는 중입니다</strong></div> : filteredPlaces.length ? filteredPlaces.map((place: any) => {
      const displayPlace: Place = { id: place.placeId, name: place.name, category: place.category || "여행 장소", address: place.address || "", image: place.personalPhotoUrl || place.imageUrl || "", description: "실제 Google Maps 검색 결과입니다.", rating: 0, reviewCount: 0, lat: place.lat ?? DEFAULT_MAP_CENTER.lat, lng: place.lng ?? DEFAULT_MAP_CENTER.lng, hours: place.hours || "", phone: "" };
      return <div className="route-saved-place-record" key={place.id}><PlaceRow place={displayPlace} onClick={() => openPlace(displayPlace)} onSave={() => void removeSavedPlace(displayPlace)} saveLabel="저장 해제" /><div className="route-saved-place-record-meta"><span>{place.note ? "개인 메모 있음" : "개인 기록을 추가해보세요"}</span><div><button onClick={() => openSavedPlaceRecordEditor(place)}><Pencil size={13} /> 기록 관리</button></div></div></div>;
  ...
  const renderMyCourses = () => {
    const isSavedTab = courseListTab === "saved";
    const savedCourses = savedCoursesQuery.data || [];
    const openSavedCourse = (course: any) => openPublicCourse({ id: String(course.id), title: course.title, region: course.region || "여행", author: "공개 코스", image: course.coverImage || mockPlaces[0].image, likes: 0, days: 1, items: [] }, "my-courses");
    const startCourseCreation = () => { setCourseStep(1); setIsCoursePublic(false); setCourseShareImageUrl(""); setCoursePhotoUrls([]); setCoursePhotoUploads([]); setScreen("course-create"); };
    const browsePublicCourses = () => { setSelectedTab("friends"); setPublicCourseFilter("all"); setScreen("public-courses"); };
    const renderLibraryEmpty = (kind: CourseListTab) => <div className={`route-course-library-empty is-${kind}`}><span className="route-course-library-empty-icon">{kind === "mine" ? <Compass size={24} /> : <Bookmark size={23} />}</span><div><strong>{kind === "mine" ? "아직 만든 코스가 없어요" : "저장한 코스가 없어요"}</strong><p>{kind === "mine" ? "마음에 드는 장소를 모아 나만의 첫 여행 코스를 만들어보세요." : "다른 여행자의 실제 코스를 살펴보고 내 여행에 저장해보세요."}</p></div><div className="route-course-library-empty-actions">{kind === "mine" ? <><button onClick={startCourseCreation}><Plus size={16} /> 첫 코스 만들기</button><button className="secondary" onClick={() => { setSelectedTab("map"); setScreen("map"); }}><MapPin size={15} /> 지도에서 장소 찾기</button></> : <button onClick={browsePublicCourses}><Compass size={16} /> 공개 코스 찾아보기</button>}</div></div>;
    return <div className="route-screen route-list-screen"><ScreenHeader title="코스" onBack={() => setTab("map")} right={!isSavedTab ? <button className="route-header-add" onClick={startCourseCreation}>+ 새 코스</button> : undefined} /><div className="route-list-tabs" role="tablist" aria-label="코스 목록"><button role="tab" aria-selected={!isSavedTab} className={!isSavedTab ? "active" : ""} onClick={() => setCourseListTab("mine")}>내 코스</button><button role="tab" aria-selected={isSavedTab} className={isSavedTab ? "active" : ""} onClick={() => setCourseListTab("saved")}>저장 코스</button></div>{isSavedTab ? <div className="route-course-list">{savedCoursesQuery.isLoading ? <div className="route-empty"><Bookmark size={22} /><strong>저장 코스를 불러오는 중입니다</strong></div> : savedCourses.length ? savedCourses.map((course: any) => <button className="route-saved-course-row" key={course.id} onClick={() => openSavedCourse(course)}><img src={course.coverImage || mockPlaces[0].image} alt="" /><span><strong>{course.title}</strong><small>{course.region || "지역 정보 없음"}</small><em>저장됨</em></span><ChevronRight size={16} /></button>) : renderLibraryEmpty("saved")}</div> : <div className="route-course-list">{courseList.length ? courseList.map((course) => <div className="route-large-course-card" key={course.id}><button className="route-course-card-main" onClick={() => { setSelectedCourse(course); setScreen("course-detail"); }}><img src={course.image} alt="" /><span><em className={`route-course-status ${course.status || "planned"}`}>{courseStatusLabel[course.status || "planned"]}</em><strong>{course.title}</strong><small>{formatCourseDateRange(course.startDate, course.endDate)} · 장소 {course.items.length}곳</small></span><ChevronRight size={16} /></button>{hasDbCourses && <button className="route-course-edit-button" aria-label="코스 수정" onClick={() => { setSelectedCourse(course); setCourseTitle(course.title); setCourseStartDate(toDateInputValue(course.startDate)); setCourseEndDate(toDateInputValue(course.endDate)); setCourseStatus(course.status || "planned"); setIsCoursePublic(Boolean((course as any).isPublic)); setCourseShareImageUrl(course.shareImageUrl || course.image); setCoursePhotoUrls(parseCoursePhotoUrls(course.photoUrls)); setCoursePhotoUploads([]); setCoursePlaces([]); setCourseStep(1); setScreen("edit-course"); }}><Pencil size={15} /></button>}</div>) : renderLibraryEmpty("mine")}</div>}</div>;
  };
  ...
  const renderProfile = () => {
    ...
    return <div className="route-screen route-profile"><ScreenHeader title="프로필" onBack={() => setScreen("friends")} right={<button aria-label="프로필 더보기" onClick={() => toast.message("공개 코스와 팔로우 정보는 이 화면에서 확인할 수 있어요.")}><MoreHorizontal size={18} /></button>} /><div className="route-profile-cover" style={{ backgroundImage: `url(${cover})` }} /><div className="route-profile-main">{profile.avatarUrl ? <img className="route-profile-avatar" src={profile.avatarUrl} alt="" /> : <div className="route-profile-avatar route-social-avatar">{(profile.name || "여").slice(0, 1)}</div>}<div className="route-profile-name-row"><div><h2>{profile.name || "여행자"}</h2><p>{profile.bio || profile.travelStyle || "여행 경험을 기록하는 Route 사용자"}</p></div>{profile.isSelf ? <button onClick={() => setScreen("mypage")}>내 프로필</button> : <button onClick={() => void toggleFollow(profile.id)} disabled={toggleFollowMutation.isPending}>{profile.isFollowing ? "팔로잉" : "팔로우"}</button>}</div><div className="route-profile-stats"><span><strong>{profile.publicCourseCount}</strong> 공개 코스</span><span><strong>{profile.followerCount}</strong> 팔로워</span><span><strong>{profile.followingCount}</strong> 팔로잉</span></div><div className="route-profile-tabs"><span className="active" aria-current="page">공개 코스</span></div><div className="route-profile-grid">{profileCourses.length ? profileCourses.map((course) => <button key={course.id} onClick={() => openPublicCourse(course)}><img src={course.image} alt="" /><strong>{course.title}</strong><small>{course.region}</small></button>) : <div className="route-empty"><Calendar size={20} /><strong>공개한 코스가 없습니다</strong></div>}</div></div></div>;
  };
  ...
  if (screen === "map") content = <>{renderMapScreen()}{isMapEntryTransitionVisible && <RouteMapEntryLoader isExiting={isMapEntryTransitionExiting} />}</>;
  else if (screen === "search") content = renderSearchScreen();
  else if (screen === "place-detail") content = renderPlaceDetailWithNavigation();
  else if (screen === "place-navigation") content = renderRouteDirectionsWithShare();
  ...
  else if (screen === "saved-courses") content = renderMyCourses();
